...
@return Promise<stream.Readable>
*/
async function streamTranscoding(transcoding, output = new PassThrough()) {
    const hls = transcoding.format.protocol.trim().toLowerCase() === "hls";
    const { url } = await fetchAPI(transcoding.url);
    if (hls) {
        const res = await request(url);
        writeHlsChunks(await res.body.text(), output)
            .catch(error => output.emit("error", error));
        return output;
    }
    else {
        undici.stream(url, res => {
            if (res.statusCode >= 400) {
                throw new Error(`${res.statusCode} ${STATUS_CODES[res.statusCode]}`);
            }
            else {
