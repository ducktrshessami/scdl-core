...
/*
*/
function playlistFromInfo(info, options) {
...
function validatePlaylist(URL) {
    URL = handleExcess(URL);
    var foo = URL.split('/');
    return ((foo.length == 6 && URL.startsWith("https://soundcloud.com/")) || (foo.length == 4 && URL.startsWith("soundcloud.com/"))) && foo[foo.length - 2].toLowerCase() === "sets";
}
...
function getPlaylistPermalink(URL) {
    if (validatePlaylist(URL)) {
        URL = handleExcess(URL);
        var foo = URL.split('/');
        return `https://soundcloud.com/${foo[foo.length - 3]}/sets/${foo[foo.length - 1]}`;
    }
}
