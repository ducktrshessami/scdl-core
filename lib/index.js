...
*/
module.exports = function scdl(URL, options) {
...
@return stream.Readable
*/
module.exports.downloadFromInfo = function downloadFromInfo(info, options) {
    const read = new stream.PassThrough();
    if (info.streamable) {
...
function findTranscoding(transcodings, options) {
    if (options.strict) { // Match all present options
        ...
        for (let i = 0; i < transcodings.length; i++) {
            ...
            if (options.preset && tc.preset.toLowerCase().trim().includes(options.preset.toLowerCase().trim())) {
                score += 1;
            }
            if (options.protocol && tc.format.protocol.toLowerCase().trim().includes(options.protocol.toLowerCase().trim())) {
                score += 1;
            }
            if (options.mimeType && tc.format.mime_type.toLowerCase().trim().includes(options.mimeType.toLowerCase().trim())) {
                score += 1;
            }
            if (options.quality && tc.quality.toLowerCase().trim().includes(options.quality.toLowerCase().trim())) {
                score += 1;
            }
