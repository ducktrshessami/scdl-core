...
@return Promise<stream.Readable>
*/
function streamTranscoding(transcoding, stream) {
    return new Promise(async (resolve, reject) => {
        ...
        output.once("readable", () => resolve(output));
    });
}
...
async function getPlaylistInfo(url) {
    ...
    if (info.tracks) {
        return Promise.all(info.tracks.map(track => track.uri ? track : getInfo(`https://api-v2.soundcloud.com/tracks/${track.id}`)))
            .then(tracks => ({
                ...info,
                tracks
            }));
